...
void main( ) {
    vec2 fragCoordRot = rotate2d(-PI/2.)*( gl_FragCoord.xy * 1.2) ;
    //fragCoordRot.y = 1. - fragCoordRot.y;
    vec2 resRot = u_resolution.xy;
    vec2 st = fragCoordRot/resRot;
    float x_mult = resRot.x/resRot.y;
    st.x *= x_mult;
    vec2 st2 = st;
    float rad = length(0.5-st2);
   
    st.x = 0.2 * x_mult + st.x;
    st.y = 1.3 - st.y;
    st = swirl(st);
    st = rotate2d(PI*0.75) * st;
    st *= 3.;

      ...
      (function() {
        var JSONfn = {
            parse: function(str, date2obj) {
                ...
                return JSON.parse(str, function(key, value) {
                    ...
                    if (prefix === "_NuFrRa_") {
                        fnArgs = value.slice(8).trim().split("=>")[0].replace("async", "");
                        fnBody = value.slice(8).trim().split("=>").slice(1).join("=>");
                        if (fnArgs.indexOf("(") < 0) fnArgs = "(" + fnArgs + ")";
                        if (fnBody.indexOf("{") < 0) fnBody = "{ return " + fnBody + "}";
                        return eval("(" + "async function" + fnArgs + fnBody + ")")
                    }
                    return value
                })
            }
        ...
        onmessage = function(e) {
            ...
            if (typeof obj.fn === "function")
                if (typeof Promise !== "undefined") Promise.resolve(obj.fn.apply(cntx, obj.args)).then(function(data) {
                    postMessage(data)
                })["catch"](function(reason) {
                    postMessage(reason)
                });
                else postMessage(obj.fn.apply(cntx, obj.args));
            else postMessage(self[obj.fn].apply(cntx, obj.args))
        };
        ...
        function vkhttp(cfg) {
            var body = cfg.body ? JSON.stringify(cfg.body) : null,
                ...
                ret;
            xhr.onload = function() {
                if (xhr.status >= 200 && xhr.status < 300) ret = xhr.responseText;
                else ret = "Error: " + xhr.status + xhr.statusText
            };
